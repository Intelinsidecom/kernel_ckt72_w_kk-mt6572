...
/******************************************************************
*msdc_early_onoff for VMCH VMC added by pengliu 20140910 for pcb2.0 hotplug
*******************************************************************/
static void msdc_early_onoff(u32 on, MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
    if (on) { // want to power on
        printk(KERN_WARNING "msdc LDO<%d> to <%d>\n", powerId,powerVolt);
...
{
    if(!on)//added by pengliu for PCBV2.0  hotplug
    {
        return 0;
	}
...
static int msdc_drv_suspend(struct platform_device *pdev, pm_message_t state)
{
    ...
    printk(KERN_INFO DRV_NAME ": msdc_drv_suspend\n");
...
#if defined(GPIO_SDHC_EINT_PIN)
	mt65xx_eint_mask(EINT_MSDC1_INS_NUM);//added by pengliu for MINI PCBV2.0 hotplut
	msdc_early_onoff(0, MT6323_POWER_LDO_VMCH, VOL_3300);//added by pengliu for MINI PCBV2.0 hotplut
#endif
    return ret;
}
...
static int msdc_drv_resume(struct platform_device *pdev)
{
    ...
    printk(KERN_INFO DRV_NAME ":msdc_drv_resume\n");
	
#if defined(GPIO_SDHC_EINT_PIN)
	mt65xx_eint_mask(EINT_MSDC1_INS_NUM);//added by pengliu for 820MINI PCBV2.0
	msdc_early_onoff(1, MT6323_POWER_LDO_VMCH, VOL_3300);//addedby pengliu for 820MINI PCBV2.0
#endif
	...
	mt65xx_eint_unmask(EINT_MSDC1_INS_NUM);//added by pengliu for 820MINI PCBV2.0  we must here. don't move
