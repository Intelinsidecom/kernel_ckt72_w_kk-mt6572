...
static void *PhoneMic_Receiver_Headset_Acoustic_Loopback(void *mPtr)
{
    ...
    while (1)
    {
        ...
        {
            ALOGD(TAG  "lls--> --------Audio_Headset_detect_thread  : Headset plug-in (%d),flag=%d\n", hds->avail,g_loopback_item);
        ...
        {
            ALOGD(TAG "lls--> --------Audio_Headset_detect_thread  : Headset plug-out (%d),flag=%d\n", hds->avail,g_loopback_item);
           // PhoneMic_Receiver_Acoustic_Loopback(Acoustic_Status - 1, &Acoustic_Status, hds->avail);
            //usleep(50000);
            //PhoneMic_Receiver_Acoustic_Loopback(Acoustic_Status, &Acoustic_Status, hds->avail);
            usleep(3000);
...
{
    struct mAudio *hds  = (struct mAudio *)mPtr;
    ALOGD(TAG "audio %s: Start", __FUNCTION__);
    usleep(100000);
    ...
    int lowFreq = freqOfRingtone * (1 - 0.99);
    ...
    while (1)
    {
        ...
        for (int i = 0; i < 3 ; i ++)
        {
            LOGE(TAG"%d.freqDataL[%d]:%d,magDataL[%d]:%d", i,i, freqDataL[i], i, magDataL[i]);
            LOGE(TAG"%d.freqDataR[%d]:%d,magDataR[%d]:%d", i,i, freqDataR[i], i, magDataR[i]);
        }
        LOGE(TAG"hds->i4OutputType :%d",hds->i4OutputType );
        LOGE(TAG"highFreq:%d,lowFreq:%d,magUpper:%d,magLower:%d",highFreq,lowFreq,magUpper,magLower);
...
#ifndef RECEIVER_HEADSET_AUTOTEST
//Receiver test
