...
static void *alsps_update_iv_thread(void *priv)
{
    ...
    while (1) {
        ...
        len = 0;
        len += snprintf(dat->info+len, sizeof(dat->info)-len, "%s: %4Xh (0:%s; +:%s) \n",uistr_info_sensor_alsps_als, lps->als_raw,uistr_info_sensor_alsps_dark,uistr_info_sensor_alsps_bright);      
        len += snprintf(dat->info+len, sizeof(dat->info)-len, "%s : %4Xh (0:%s ; +:%s) \n",uistr_info_sensor_alsps_ps, lps->ps_raw,uistr_info_sensor_alsps_far,uistr_info_sensor_alsps_close);
