...
void mAudio_receiver_debug_playtone(struct mAudio *mc, void *priv)
{
    pthread_create(&mc->hHeadsetThread, NULL, Audio_Receiver_Playabck_thread, priv);
}

void mAudio_receiver_debug_stoptone(struct mAudio *mc)
{
    ...
    do
    {
        ...
        switch (chosen)
        {
            case ITEM_RINGTONE:
                ...
                mAudio_receiver_debug_playtone(mc, priv);
            ...
            case ITEM_MIC1:
                ...
                {
                    mc->exit_thd = true;
                    mAudio_receiver_debug_stoptone(mc);
                    mc->exit_thd = false;
                }
                usleep(3000);
                PhoneMic_Receiver_Loopback(MIC1_ON);  // enable Receiver MIC1 loopback
