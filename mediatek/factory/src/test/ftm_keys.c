...
//#define LYS_ADD_ITEM //use it for item add
#define mod_to_keys(p)	(struct keys *)((char *)(p) + sizeof(struct ftm_module))
#ifdef LYS_ADD_ITEM
static DEFINE_KEYS_KEYMAP(keys_keymap_s);
#define KEYS_NUM_KEYS	((int)(sizeof(keys_keymap_s) / sizeof(keys_keymap_s[0])))+1
struct key keys_keymap[KEYS_NUM_KEYS];
#else
...
static int keys_entry(struct ftm_param *param, void *priv)
{
	...
	for (i = 0; i < KEYS_NUM_KEYS; i++) {
        ...
        LOGD(TAG"i=%d code=%d,name=%s length=%d",i,keys_keymap[i].code ,keys_keymap[i].name,KEYS_NUM_KEYS );
...
int keys_init(void)
{
	...
	if (!keys->iv)
        ...
        #ifdef LYS_ADD_ITEM
        //init key map by yongsheng
       int i;
	struct key voldown = {KEY_VOLUMEDOWN,"VLDown"};
        for(i=0 ;i<KEYS_NUM_KEYS-1;i++){
		keys_keymap[i] = keys_keymap_s[i];
		 LOGD(TAG"original : i=%d code=%d,name=%s length=%d",i,keys_keymap_s[i].code ,keys_keymap_s[i].name,KEYS_NUM_KEYS );
	}
	keys_keymap[KEYS_NUM_KEYS-1] = voldown;
	#endif
