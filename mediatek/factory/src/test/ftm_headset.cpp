...
static void * Audio_Record_thread(void *mPtr)
{
    ...
    ALOGD(TAG "headset %s: Start", __FUNCTION__);
    usleep(100000);
...
//    bool dumpFlag = true;//for test
    ...
    while (1) {
       ...
       for(int i = 0 ; i < 4096 ; i++)
       {
        ...
        LOGE(TAG"hds->isPhoneTest:%d,headset_phonetest_state:%d",hds->isPhoneTest,headset_phonetest_state);*/
        if(hds->isPhoneTest)
        {
