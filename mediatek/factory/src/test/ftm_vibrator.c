...
static void *update_vibrator_thread_default(void *priv)
{
	...
	{
	do {
        	write_int(VIBRATOR_ENABLE, 2000); // work 2s modified by zms
		sleep(2);
		write_int(VIBRATOR_ENABLE, 0); // stop 1 seconds add by zms
		sleep(1);
		if (vibrator_test_exit)
			break;
		sleep(1);
...
static void * Audio_Record_thread(void *mPtr)
{
    ...
    ALOGD(TAG "vibrator %s: Start", __FUNCTION__);
    usleep(100000);
...
//    dumpFlag=true;//for test
    ...
    while (1) {
       ...
       /* int j;
		for(j = 0;j < 3 ;j ++)
        {
            LOGE(TAG"freqDataL[%d]:%d,magDataL[%d]:%d",j,freqDataL[j],j,magDataL[j]);
            LOGE(TAG"freqDataR[%d]:%d,magDataR[%d]:%d",j,freqDataR[j],j,magDataR[j]);
        }
