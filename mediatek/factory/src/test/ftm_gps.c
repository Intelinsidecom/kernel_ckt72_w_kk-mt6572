...
static item_t gps_items[] = {
    item(ITEM_PASS,   uistr_pass),// open by jy
    item(ITEM_FAIL,   uistr_fail),// open by jy
    item(ITEM_HTTFF,  uistr_info_gps_hot_restart),// open by jy
    item(ITEM_CTTFF,  uistr_info_gps_cold_restart),// open by jy
    item(-1, NULL),
};
...
int gps_entry(struct ftm_param *param, void *priv)
{
	...
	iv->redraw(iv);
#ifdef CKT_SUPPORT_AUTOTEST_MODE //jy added 	
    do{
