...
static int textview_redraw(struct textview *tv)
{
    ...
    if (tv->m_text) {
        ...
        for (line = tv->m_start; line < tv->m_end; line++) {
			...
			LOGD(TAG"line : %d,drawstring : %s",line,tv->m_pstr[line]);
        }
    }
	LOGD(TAG"------textview_redraw----fack draw--_>");
...
static int itemview_redraw(struct itemview *iv)
{
    ...
    if (ptext) {
	LOGD(TAG"draw string ptext:%s",ptext);
        for (ptr = ptext->string; *ptr != '\0'; ++ptr) {
	    LOGD(TAG"current col:%d,row:%d,char:%c,word_cnt:%d",text_col,text_row,*ptr ,word_cnt);
//leibo add for FUTURE-919
#ifdef SUPPORT_GB2312
            if (*ptr == '\n' || word_cnt  >= (text_cols *2  - 8)) //add * 2
#else
            if (*ptr == '\n' || word_cnt  >= (text_cols))
#endif
//leibo add end
...
int paintview_redraw(struct paintview *pv)
{
    ...
    draw_background_locked(gCurrentIcon);
	LOGD(TAG"------imageview_redraw----fack draw--_>");
