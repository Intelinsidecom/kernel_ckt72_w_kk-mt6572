...
static item_t ftm_menu_items[] = {
  ...
  //  item(ITEM_CLREMMC,  uistr_clear_emmc),
    ...
    item(ITEM_REBOOT,    uistr_reboot),
     item(ITEM_BOOT,    uistr_boot),
    item(ITEM_MAX_IDS,   NULL),
};

static item_t ftm_auto_test_items[] = {
#ifdef FEATURE_FTM_TOUCH
	//item(ITEM_TOUCH_AUTO,	uistr_touch_auto), //modefied by leibo
#endif
...
static item_t ftm_test_items[] = {
    ...
    //item(ITEM_TOUCH_AUTO,	uistr_touch_auto), // modified by leibo
#endif
#ifdef FEATURE_FTM_LCD
    item(ITEM_LCD,     uistr_backlight_level),
#endif
#ifdef FEATURE_FTM_AUDIO
//    item(ITEM_RINGTONE_1, uistr_info_audio_ringtone),
#endif
#ifdef FEATURE_FTM_HALL
	item(ITEM_HALLSENSOR, Hall_Sensor),
#endif
...
if(is_support_modem(1)){
    ...
    if(strlen(barcode) < 62)
    {
        LOGD(TAG "have no modem calibration\n");
    ...
    else
    {
        ...
        {
                LOGD(TAG "calibration pass\n");
                calibration_status = 1;
        }
        else if ('0' == barcode[60] && '1' == barcode[61])
        {
                LOGD(TAG "calibration failed\n");
                calibration_status = 2;
        }
        else
        {
                LOGD(TAG "calibration unknown\n");
...
//    print_verinfo(info, &len,  "UBoot Ver.  ", uboot_ver);
    ...
    print_verinfo(info, &len,  "Calibration Status.", (calibration_status == 1)?"OK":"Fail");
    return 0;
}
