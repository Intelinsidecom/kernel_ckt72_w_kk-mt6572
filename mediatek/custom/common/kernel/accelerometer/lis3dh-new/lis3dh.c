...
u8 filter_counter = 0;
static int LIS3DH_Filter(s16 data[LIS3DH_AXES_NUM])
{
	#define STACK_NUM 10
	u8 i = 0;
	s32 x_temp = 0;
	...
	static s16 x_stack[STACK_NUM] = {0};
	static s16 y_stack[STACK_NUM] = {0};
	static s16 z_stack[STACK_NUM] = {0};

	if (data == NULL) {
		return -1;
	}

	if (filter_counter < STACK_NUM) {
		x_stack[filter_counter] = data[0];
		y_stack[filter_counter] = data[1];
		z_stack[filter_counter++] = data[2];

		for (i = 0; i < filter_counter; i++) {
			x_temp += x_stack[i];
			y_temp += y_stack[i];
			z_temp += z_stack[i];
		}

		x_temp /= filter_counter;
		y_temp /= filter_counter;
		z_temp /= filter_counter;
	...
	x_stack[STACK_NUM - 1] = data[0];
	y_stack[STACK_NUM - 1] = data[1];
	z_stack[STACK_NUM - 1] = data[2];

	x_temp += x_stack[STACK_NUM - 1];
	y_temp += y_stack[STACK_NUM - 1];
	z_temp += z_stack[STACK_NUM - 1];
...
static int LIS3DH_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		LIS3DH_Filter(obj->data);//add by zms 
