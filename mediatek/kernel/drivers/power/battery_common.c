...
extern  kal_uint32 g_charging_enable;//pengliu added
int g_charging_temperature = 0;  //add zms 
...
//add by wenliang for disable charging
#define PROC_FAN5405      "disable_charging"
char fan5405_ce = 0;
//end
...
extern int IMM_auxadc_GetOneChannelValue_Cali(int Channel, int*voltage); // added by leibo for Doro Cradle changer
static struct switch_dev cradle_sdev; // cradle switch device, added by leibo.
...
#define ADC0_USB_ID_CHANNEL 0 //Channel 0 for USB ID check
#define CRADLE_PLUG_OUT 0
#define CRADLE_PLUG_IN 1
static int cradle_plug; // cradle plug in/out flag.
...
static void ac_update(struct ac_data *ac_data)
{
    ...
    {
		//added by lb for Doro Cradle detect.
		IMM_auxadc_GetOneChannelValue_Cali(ADC0_USB_ID_CHANNEL, &voltage);
		printk("%s ADC0_USB_ID channel voltage: %d----\n", __func__, voltage);
		if ((BMT_status.charger_type == NONSTANDARD_CHARGER) &&
		   ((voltage >= 500000) && (voltage <= 650000))) //0.5v ~ 0.65v for Doro Cradle changer
		{
			printk("%s into Doro Cradle!!!\n", __func__);
			ac_data->AC_ONLINE = 1;
			ac_psy->type = POWER_SUPPLY_TYPE_MAINS;	

			cradle_plug = CRADLE_PLUG_IN; //leibo
			switch_set_state((struct switch_dev *)&cradle_sdev, cradle_plug);
		}
		//added end
...
//add by zms
	...
	{
		g_charging_temperature = 1;
		
	}
	else {
		 g_charging_temperature = 0;  		
	}//end
...
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
	else if (BMT_status.temperature <= MIN_CHARGE_TEMPERATURE_NOTIFY)
    {
        g_BatteryNotifyCode |= 0x0020;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too low)\n", BMT_status.temperature);
    }
...
#if 0
static struct task_struct *cradle_check_task = NULL;
static int cradle = 0;
ssize_t print_cradle_state(struct switch_dev *sdev, char *buf)
{
	//switch_set_state((struct switch_dev *)&cradle_sdev, cradle_plug);
	cradle = 1;
	return sprintf(buf, "%d\n", sdev->state);
...
static int cradle_check_kthread(void *data)
{
	
	static int num = 0;//for test
    ...
    for( ;; ) {
        ...
        if(cradle == 1)
        {
			cradle_plug = (num++)%2 ? CRADLE_PLUG_OUT : CRADLE_PLUG_IN;// for test
			switch_set_state((struct switch_dev *)&cradle_sdev, cradle_plug);
			cradle = 0;
        }
    }
...
#if 1
static int fan_proc_cmd_read(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
	buf[0] = fan5405_ce;
	return 1;
}	
static int fan_proc_cmd_write(struct file *file, const char *buf, unsigned long count, void *data)	
{
    char cmd_buf[256];
    int ret;
    if (count == 0)
    ...
    if(ret < 0)
         return -1;
    fan5405_ce = cmd_buf[0];
    //fan5405_set_ce(fan5405_ce);
    return count;
}
void fan_proc_init(void)	
{
   struct proc_dir_entry *fan5405 = create_proc_entry(PROC_FAN5405,0666,0);
   printk("[PROC_FAN5404] Initialize proc\n");
   fan5405->read_proc = fan_proc_cmd_read;
   fan5405->write_proc = fan_proc_cmd_write;
}
#endif
...
#endif
	/******************Doro Cradle Switch Device Register***************/
	cradle_sdev.name = "cradle";
	cradle_sdev.index = 0;
	cradle_sdev.state = CRADLE_PLUG_OUT; //need modify.
	//cradle_sdev.print_state = &print_cradle_state;
	ret = switch_dev_register(&cradle_sdev);
	if(ret)
	{
		printk("[Cradle]switch_dev_register returned:%d!\n", ret);
		return 1;
	}
#if 0	//test for cradle
	cradle_check_task = kthread_create(cradle_check_kthread, NULL, "cradle_check_kthread");
	
	if (IS_ERR(cradle_check_task)) 
	{
		printk("cradle check task create fail\n");
	}
	else 
	{
		wake_up_process(cradle_check_task);
	}
#endif
	/**************************added by leibo***********************************/
	g_bat_init_flag = KAL_TRUE;
